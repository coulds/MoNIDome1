...
public class PublicAccountAdapter extends RecyclerView.Adapter<PublicAccountAdapter.MViewHold> {

    private Context mcontext;
    private List<PADecBean.DataBean.DatasBean> mdata;
    public PublicAccountAdapter(Context context,List<PADecBean.DataBean.DatasBean> data){
    ...
    public MViewHold onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mcontext).inflate(R.layout.wechar_item,parent,false);
        return new PublicAccountAdapter.MViewHold(view);
    }
...
//        holder.mProjectImageView.setVisibility(View.VISIBLE);
        ...
        holder.mItemTop.setText(mdata.get(position).getSuperChapterName());
//        Glide.with(mcontext).load(mdata.get(position).getEnvelopePic()).into(holder.mProjectImageView);
        holder.mProjectType.setText("分类："+mdata.get(position).getSuperChapterName()+"/"+mdata.get(position).getChapterName());
    }
    ...
    public class MViewHold extends RecyclerView.ViewHolder {
        ...
        public MViewHold(@NonNull View itemView) {
            super(itemView);
            mProjectAuthor = itemView.findViewById(R.id.item_home_author);
            mProjectTitle = itemView.findViewById(R.id.item_home_content);
            mProjectType = itemView.findViewById(R.id.item_article_type);
            mProjectDate = itemView.findViewById(R.id.item_home_date);

//            mProjectContent = itemView.findViewById(R.id.item_project_content);
            mItemTop = itemView.findViewById(R.id.item_home_new);
